...
覆盖 **文档解析 → 向量检索 → 生成 → 幻觉自检** 全链路的 RAG 问答系统，引入 Multi-Query 多路召回、重排、基于 LLM 的幻觉自检与条件分支重试等检索增强策略。

| 维度 | 说明 |
|---|---|
| 检索增强 | Multi-Query 多路召回 + 重排 + 查询改写 |
...
关键配置项：
```ini
DEEPSEEK_API_KEY=your_siliconflow_api_key_here      # LLM 生成（硅基流动密钥）
OPENAI_API_KEY=your_siliconflow_api_key_here        # Embedding（硅基流动密钥，与上者通常相同）
CHUNK_SIZE=500         # 分块大小
...
| `utils/file_loader.py` | 文件解析：PDF/TXT/MD/JSON/CSV → `Document`，异常捕获 |
...
2. **Multi-Query 多路召回**：一个问题生成多个角度子查询，分别检索后去重合并，扩大召回覆盖（`reranker_helper.multi_query_generate`）。
3. **重排（Reranker）**：召回（Bi-Encoder）追求速度和召回率但打分粗；重排对 query-doc 精打分，提升 top-K 相关性。优先走硅基流动 `/rerank` API（模型 `BAAI/bge-reranker-v2-m3`），不可用时降级本地 CrossEncoder（`reranker_helper.rerank_documents`）。
4. **幻觉自检条件分支**：生成后用 LLM 检测回答是否与上下文冲突或编造；发现幻觉则通过 LangGraph 条件边跳回重生成，并有 `max_retry` 上限防死循环。
...
- **Embedding 质量**：默认 SimpleEmbeddings（哈希）仅用于无密钥演示，生产需通过硅基流动（OpenAI 兼容协议）或 HuggingFace 使用真实语义向量。
- **重排模型**：当前重排走硅基流动 `/rerank` API（`BAAI/bge-reranker-v2-m3`）；如需完全离线部署，需预下载 CrossEncoder 模型。
